...
const flatter = (arrayOfArrays) =>{
    let flattenArray = [];

    arrayOfArrays.reduce((accumulator, currentValue) =>{
    flattenArray = flattenArray.concat(currentValue);
}, 0);
...
const every = (array, test) =>{
   const result = array.filter(test);
   console.log(result)
   if(result.length === array.length) return true;
   return false;
}

const everyVersion2 = (array, test) =>{
 ...
 const everyVersion3 = (array, test) =>{
    if(array.length)
...
function characterScript(code) {
  ...
  function dominantDirection(text) {
    ...
    let sortedScripts = scripts.sort((a, b) => b.count - a.count)
